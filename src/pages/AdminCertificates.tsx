  ...
  return (
    <div className="flex items-center gap-3">
      ...
      <div className="flex flex-col gap-1">
        <button onClick={download} className="text-[11px] text-bronze hover:underline text-left">
          Download QR
        </button>
        <button
          onClick={() => {
  ...
  if (checking) {
    return (
      <div className="min-h-screen bg-background">
        <SieveNav />
        <div className="container pt-40 text-soft text-sm">Loading registry…</div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <PageHero
        ...
        intro="Generate a unique code and QR verification link for each internship certificate. The QR can be printed directly on the document; anyone scanning it sees the authenticated record."
      />
      ...
      <section className="py-8 border-b border-border">
        <div className="container flex flex-wrap items-center gap-3">
          ...
          <button onClick={signOut} className="text-[13px] text-soft hover:text-ink ml-auto">
            Sign out
          </button>
        </div>
      </section>
        ...
        <section className="py-10 border-b border-border">
          <div className="container">
            <div className="border border-border bg-paper-raised p-7 max-w-2xl">
              <h2 className="font-display text-xl text-ink mb-2">Administrator access required</h2>
              <p className="text-soft text-sm leading-relaxed">
                Your account is signed in but does not hold the administrator role, so issuing and
                revoking certificates is disabled. An existing administrator must grant the role
                before you can write to the registry.
              </p>
            </div>
          </div>
        </section>
      ...
      {isAdmin && (
        <section className="py-14 border-b border-border">
          <div className="container">
            <p className="kicker mb-8">Issue certificate</p>
            <form onSubmit={issue} className="grid grid-cols-1 md:grid-cols-3 gap-6 max-w-4xl">
              {[
              ...
              <div className="md:col-span-3 space-y-2">
                <Label htmlFor="notes" className="text-[10px] uppercase tracking-[0.14em] text-soft">
                  Notes (optional)
                </Label>
                <Textarea
                  id="notes"
              ...
              <div>
                <Button type="submit" variant="cyber" className="h-11 px-6 text-xs" disabled={busy}>
                  Issue certificate
                </Button>
              </div>
            </form>
      ...
      <section className="py-14">
        <div className="container">
          ...
          <div className="border-t border-border">
            {rows.length === 0 && (
              <p className="text-soft text-sm py-6">No certificates issued yet.</p>
            )}
            {rows.map((row) => (
              <div
                key={row.id}
              ...
              >
                ...
                <div className="md:col-span-3">
                  ...
                  <p className="text-[11px] text-soft">{row.role_title ?? "—"}</p>
                </div>
                <div className="md:col-span-2">
                  <span
                    className={`font-mono-data text-[10px] tracking-[0.12em] border border-border px-2 py-0.5 ${
                      row.status === "valid" ? "text-bronze" : "text-destructive"
                    }`}
                  >
